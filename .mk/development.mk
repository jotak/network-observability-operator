...
set-plugin-image:
ifeq ("", "$(CSV)")
	kubectl set env -n $(NAMESPACE) deployment netobserv-controller-manager -c "manager" RELATED_IMAGE_CONSOLE_PLUGIN=$(PLUGIN_IMAGE)
	kubectl set env -n $(NAMESPACE) deployment netobserv-controller-manager -c "manager" RELATED_IMAGE_CONSOLE_PLUGIN_PF5=$(PLUGIN_IMAGE)
	kubectl set image deployment/netobserv-plugin-static netobserv-plugin-static=$(PLUGIN_IMAGE)
else
	./hack/swap-image-csv.sh $(CSV) $(OPERATOR_NS) console-plugin RELATED_IMAGE_CONSOLE_PLUGIN $(PLUGIN_IMAGE)
	./hack/swap-image-csv.sh $(CSV) $(OPERATOR_NS) console-plugin-pf5 RELATED_IMAGE_CONSOLE_PLUGIN_PF5 $(PLUGIN_IMAGE)
endif
	@echo -e "\n==> Redeploying..."
	kubectl rollout status -n $(OPERATOR_NS) --timeout=60s deployment netobserv-controller-manager
	kubectl wait -n $(OPERATOR_NS) --timeout=60s --for condition=Available=True deployment netobserv-controller-manager
