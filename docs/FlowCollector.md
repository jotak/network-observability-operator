...
Otherwise it is matched as a case-sensitive string.<br/>
        ...
        <td>
          Privileged mode for the eBPF Agent container. When ignored or set to `false`, the operator sets
granular capabilities (BPF, PERFMON, NET_ADMIN, SYS_RESOURCE) to the container, for the principle of least privilege.
Some agent features require the privileged mode, such as packet drops tracking (see `features`) and SR-IOV support.
It is required to listen on secondary interfaces.<br/>
        </td>
        <td>false</td>
      </tr><tr>
