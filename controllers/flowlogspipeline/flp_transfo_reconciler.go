...
func (r *flpTransformerReconciler) reconcile(ctx context.Context, desired *flowslatest.FlowCollector) error {
	...
	if r.AvailableAPIs.HasConsoleConfig() {
		if err := reconcileDashboardConfig(ctx, &r.ClientHelper, dbConfigMap); err != nil {
			return err
		}
	}
	...
	if _, err = r.Watcher.ProcessCACert(ctx, r.ClientHelper, &desired.Spec.Loki.TLS, r.Namespace); err != nil {
		return err
	}
...
func (r *flpTransformerReconciler) reconcilePrometheusService(ctx context.Context, builder *transfoBuilder) error {
	...
	if !r.Managed.Exists(r.owned.promService) {
	...
	if r.AvailableAPIs.HasPromRule() {
		...
		if err := reconcilers.GenericReconcile(ctx, r.Managed, &r.ClientHelper, r.owned.prometheusRule, promRules, &report, helper.PrometheusRuleChanged); err != nil {
			return err
		}
	}
	return nil
