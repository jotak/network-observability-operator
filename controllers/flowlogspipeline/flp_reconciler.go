...
func reconcileDashboardConfig(ctx context.Context, cl *helper.ClientHelper, dbConfigMap *corev1.ConfigMap) error {
	if dbConfigMap == nil {
...
func annotateKafkaCerts(ctx context.Context, info *reconcilers.Common, spec *flowslatest.FlowCollectorKafka, prefix string, annotations map[string]string) error {
	caDigest, userDigest, err := info.Watcher.ProcessMTLSCerts(ctx, info.ClientHelper, &spec.TLS, info.Namespace)
	if err != nil {
		return err
	}
	if caDigest != "" {
	...
	if helper.UseSASL(&spec.SASL) {
		saslDigest, err := info.Watcher.ProcessSASL(ctx, info.ClientHelper, &spec.SASL, info.Namespace)
		if err != nil {
			return err
		}
		if saslDigest != "" {
