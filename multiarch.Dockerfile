# Note about multiarch: due to incompatibilities between docker and podman, this file is not to be used as is, but to be sed'ed first, replacing $TARGETARCH with the desired architecture.
# Example:
# sed 's~$TARGETARCH~arm64~g' multiarch.Dockerfile | podman buildx build -f - .
