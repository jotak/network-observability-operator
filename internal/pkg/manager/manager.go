...
func NewManager(
	...
	// bundled CRBs grant permissions to SAs in this namespace, so it must exist
	// before any unprivileged user can create it.
	if err := internalManager.Add(manager.RunnableFunc(func(ctx context.Context) error {
		// TODO take that namespace from ENV, defaulting to netobserv
		nsName := constants.DefaultOperatorNamespace
		ns := &corev1.Namespace{}
		if err := internalManager.GetClient().Get(ctx, types.NamespacedName{Name: nsName}, ns); err != nil {
			...
			return internalManager.GetClient().Create(ctx, &corev1.Namespace{
